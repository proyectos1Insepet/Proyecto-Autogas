...
function print_copy(){
	...
	pg.connect(conString, function(err, client, done){                  //conectar a la base de datos
        if(err){
			...
			client.query(sprintf("SELECT producto, volumen, dinero, precio, idestacion, serial,km, autorizacion, id_venta,fecha,nombrecuenta,direccion,telefono,placa FROM venta  WHERE id = (SELECT MAX(id) FROM venta WHERE cara ='%1$s');", cara), function(err,result){
				done();
				if(err){
					return console.error('error de envio recuperada', err); 
				}else{					
					...
					var autorizacioncopy = result.rows[0].autorizacion;	
...
function watchful(){
    ...
    var f = new Date();
    if((f.getHours()=='20')&&(f.getMinutes()=='40')&&(corte_ok==0)){
    ...
    if((f.getHours()=='21')&&(f.getMinutes()=='0')&&(corte_ok==0)){
        printport.write('MOMENTO DE CORTE\n');
        printport.write('REALICE CIERRE DE TURNO\n');
        printport.write('PARA INICIAR VENTA\n\n\n\n\n\n');              //A la hora programada se ejecuta la funcion para obligar a corte
        printport.write('*** CORTE PROGRAMADO***\n');
    ...
    if((f.getHours()=='21')&&(f.getMinutes()=='20')&&(corte_ok==0)){
        printport.write('MOMENTO DE CORTE\n');
        printport.write('REALICE CIERRE DE TURNO\n');
        printport.write('PARA INICIAR VENTA\n\n\n\n\n\n');              //A la hora programada se ejecuta la funcion para obligar a corte
        printport.write('*** CORTE PROGRAMADO***\n');
